...
class CAsyncRoute extends React.Component {
        ...
        if (((this.state.exception && nextState.exception)) || (this.props || nextProps)) {
            return (this.state.exception !== null) ? true :
                (() => {
                    ...
                    CAsyncRoute.stack[Symbol("stack")] = nextState.exception || performWork;
    ...
    static router_factory(i, menu, mr, base_path, fallback) {
        ...
        if (component || render || children) {
            ...
            if (asroute_props.renderFactory === "render") {
                ...
                const closure = function (__asroute_props) {
                    function _func_(route_component_props) {
                        class CompInner extends React.Component {
                            constructor(props) {
                                super(props);
                            }
                            render() {
                                return React.createElement(CAsyncRoute, Object.assign({}, route_component_props, __asroute_props));
                            }
                        }
                        let __ = CAsyncRoute.components_cache.get(route_props.path);
                        if (!__) {
                            __ = React.createElement(CompInner, null);
                            CAsyncRoute.components_cache.set(route_props.path, __);
                        }
                        return __;
                    }
                    return _func_;
                };
                return React.createElement(react_router_dom_1.Route, Object.assign({ key: i }, route_props, { component: closure(asroute_props) }));
            }
            else {
                return React.createElement(react_router_dom_1.Route, Object.assign({ key: i }, route_props, { children: (rest) => React.createElement(CAsyncRoute, Object.assign({}, rest, asroute_props)) }));
            }
        }
    }
    static dynamic_renders_routers(subMenus, mapping_routes, { with_switch = {}, with_redirect = {}, base_path = "" } = {}) {
        ...
        subMenus.forEach((v, i) => {
            if (!mapping_routes[v.page]) {
                if (mapping_routes["*"]) {
                    routes.push(CAsyncRoute.router_factory(i, null, null, "", Object.assign({}, mapping_routes["*"], { path: gen_path(v.page, base_path) })));
                }
                else if (CAsyncRoute.g_cfg) {
                    const route = CAsyncRoute.g_cfg.get("*");
                    if (route) {
                    }
                    else {
                ...
                routes.push(CAsyncRoute.router_factory(i, v, mapping_routes, base_path));
            }
        });
        if (with_redirect && mapping_routes[subMenus[0].page]) {
            const redirect = CAsyncRoute.redirect_factory(subMenus[0], mapping_routes, base_path, routes.length);
            if (redirect) {
    ...
    static global_config(cfg) {
        if (!CAsyncRoute.g_cfg)
            CAsyncRoute.g_cfg = new Map();
        if (cfg["*"]) {
            CAsyncRoute.g_cfg.set("*", cfg["*"]);
        }
        return CAsyncRoute.g_cfg;
    }
}
CAsyncRoute.stack = new Proxy([], {
    get(t, p, r) {
        return t[p];
    },
    set: rc_recivers_1.default
});
CAsyncRoute.components_cache = new Map();
